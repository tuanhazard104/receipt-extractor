...
def group_text_boxes_contour_based(img, heatmap, text_boxes):
    ...
    boxes, lines = get_group_text_line(heatmap, text_boxes)
    merged_boxes = []
    for line_id in range(max(lines) + 1):
        indexes = np.where(lines == line_id)[0]
        if len(indexes) == 0:
            continue
        filter_boxes = [boxes[i] for i in indexes]
        filter_boxes = sorted(filter_boxes, key=lambda x: np.mean(x[::2]))
        merged_box = merge_boxes(filter_boxes)
