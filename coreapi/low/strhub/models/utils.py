...
def _get_config(experiment: str, **kwargs):
    ...
    with open("coreapi/low/strhub/configs/main.yaml", 'r') as f:
        config = yaml.load(f, yaml.Loader)['model']
    with open("coreapi/low/strhub/configs/charset/94_full.yaml", 'r') as f:
        config.update(yaml.load(f, yaml.Loader)['model'])
    with open(f'coreapi/low/strhub/configs/experiment/{experiment}.yaml', 'r') as f:
        exp = yaml.load(f, yaml.Loader)
    # Apply base model config
    model = exp['defaults'][0]['override /model']
    with open(f'coreapi/low/strhub/configs/model/{model}.yaml', 'r') as f:
...
def create_model(experiment: str, pretrained: bool = False, **kwargs):
    ...
    if pretrained:
        pretrained_weights = get_pretrained_weights(experiment)
